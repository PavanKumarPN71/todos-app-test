...
// it.skip("should update the database with the new data sent -api/todos/:id",()=>{
//     const todo = new TodoModel({label : "Banking", status : true})
//     todo.save().then(response => {
//         chai.request(server)
...
//                 expect(res.body).not.to.be.undefined;
//                 expect(res.body._id.toString()).to.equal(response._id.toString())
//                 //expect(res.body.label).to.be.equal("shopping")
//                 //expext(res.body.status).to.be.equal(false)
//             })
//     }).catch(console.log)
